...
mcdm_weights = {
    "rain": 0.25,  # High impact due to concentrated monsoon rainfall
    "slope": 0.20,  # Significant influence on runoff velocity and volume
    "geom": 0.15,  # Affects subsurface water movement and storage
    "lulc": 0.15,  # Determines infiltration and evapotranspiration rates
    "soilTx": 0.10,  # Soil texture influences infiltration capacity
    "soil_moisture": -0.2,  # Higher moisture reduces infiltration potential
    "runoff_coeff": 0.05,  # Represents combined effect of land cover and soil
    "flow_acc": 0.05,  # Indicates potential accumulation zones
    "dist_to_water": -0.15,
}

band_cmaps = {
    "dist_to_water": "YlGnBu",  # Light to dark blue-green (distance)
    "flow_acc": "cubehelix",  # Perceptually uniform for flow accumulation
    "geom": "tab10",  # Categorical colors (e.g., geomorphology types)
    "lulc": "terrain_r",  # Categorical colormap for land cover classes
    "ndwi": "BrBG",  # Diverging colormap for water index
    "rain": "Blues",  # Shades of blue for rainfall
    "runoff_coeff": "YlOrRd",  # Yellow-orange-red for runoff potential
    "slope": "summer",  # Good for elevation and slope representation
    "soilTx": "managua",  # Categorical colors for soil texture
    "soil_moisture": "BuGn",  # Blue-green for moisture
    "stream_order": "BuPu",  # Perceptually uniform for stream order
}
categorical_labels = {
    "lulc": {
        10: "Tree Cover",
